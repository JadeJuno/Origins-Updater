...
TRACE_TYPE = dict[str, Any]


def log(log_type: str, trace: TRACE_TYPE, text: str):
    try:
        file = trace["file"]
    except KeyError:
        file = ""
    if "fields" in trace and trace["fields"] != "":
        fields = trace["fields"]
        print(f"[{log_type}] File: {file} in {fields}: {text}")
    else:
        print(f"[{log_type}] File: {file}: {text}")


K = TypeVar("K")
V = TypeVar("V")  # Damn TypeVars suck...
def rename_key(target_dict: dict[K, V], old_key: K, new_key: K) -> dict[K, V]:
...
def get_items_from_folder(folder_path: Path) -> tuple[list[str], list[str]]:
...
def get_items_from_all_folders(folder_path: Path) -> tuple[list[str], list[str]]:
    """
...
def is_datapack_valid(folder_path: Path) -> bool:
...
def get_type(json_data: dict[str, str | Any]) -> str:
    data_type = json_data["type"]

    if data_type.startswith("apoli:"):
        data_type = data_type.replace("apoli:", "origins:", 1)

    return data_type


def unzip_datapack(zip_path: Path, extracted: Path):
    """Unzips the zip."""
...
def get_namespaces(folder_path: Path) -> list[str]:
    folders, _ = get_items_from_folder(folder_path)
    return folders


def fix_damage(trace: TRACE_TYPE, data_type: str, json_data: dict[str, Any]):
...
def fix_meta_action(trace: TRACE_TYPE, action_type: str, json_data: dict[str, Any]):
    if action_type == "origins:chance":
        if "action" in json_data:
            json_data = rename_key(json_data, "action", "success_action")
            log("INFO", trace, "Renamed action to success_action")


def fix_entity_action(trace: TRACE_TYPE, json_data: dict[str, Any]):
    action_type = get_type(json_data)
    if action_type in docactions.meta_actions:
        fix_meta_action(trace.copy(), action_type, json_data)
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.meta_actions, "Entity")
    elif action_type in docactions.entity_actions:
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.entity_actions)
        if action_type == "origins:action_on_set":
        ...
        if action_type == "origins:spawn_effect_cloud":
        ...
        if action_type == "origins:damage":
            fix_damage(trace, action_type, json_data)


def fix_bientity_action(trace: TRACE_TYPE, json_data: dict[str, Any]):
    action_type = get_type(json_data)
    if action_type in docactions.meta_actions:
        fix_meta_action(trace.copy(), action_type, json_data)
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.meta_actions, "Bi-entity")
    elif action_type in docactions.bientity_actions:
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.bientity_actions)
        if action_type == "origins:add_to_set":
            json_data["type"] = "origins:add_to_entity_set"
            log("INFO", trace, "Renamed add_to_set to add_to_entity_set")
        if action_type == "origins:remove_from_set":
...
def fix_block_action(trace: TRACE_TYPE, json_data: dict[str, Any]):
    action_type = get_type(json_data)
    if action_type in docactions.meta_actions:
        fix_meta_action(trace.copy(), action_type, json_data)
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.meta_actions, "Block")
    elif action_type in docactions.block_actions:
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.block_actions)


def fix_item_action(trace: TRACE_TYPE, json_data: dict[str, Any]):
    action_type = get_type(json_data)
    if action_type in docactions.meta_actions:
        fix_meta_action(trace.copy(), action_type, json_data)
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.meta_actions, "Item")
    elif action_type in docactions.item_actions:
        iterate_through_fields(trace.copy(), action_type, json_data, docactions.item_actions)
        if action_type == "origins:merge_nbt":
            json_data["type"] = "origins:merge_custom_data"
            log("INFO", trace, "Renamed item action type merge_nbt to merge_custom_data")


def fix_meta_condition(trace: TRACE_TYPE, condition_type, json_data: dict[str, Any]):
    ...


def fix_entity_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Entity")
    elif condition_type in docconditions.entity_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.entity_conditions)
        if condition_type == "origins:entity_group":
        ...
        if condition_type == "origins:set_size":
            json_data["type"] = "origins:entity_set_size"
            log("INFO", trace, "Renamed set_size to entity_set_size")


def fix_bientity_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Bi-entity")
    elif condition_type in docconditions.bientity_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.bientity_conditions)
        if condition_type == "origins:in_set":
            json_data["type"] = "origins:in_entity_set"
            log("INFO", trace, "Renamed in_set to in_entity_set")


def fix_block_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Block")
    elif condition_type in docconditions.block_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.block_conditions)
        if condition_type == "origins:replacable":
            json_data["type"] = "origins:replaceable"
            log("INFO", trace, "Renamed replacable block condition to replaceable (typo)")
        if condition_type == "origins:material":
...
def fix_item_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Item")
    elif condition_type in docconditions.item_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.item_conditions)
        if condition_type == "origins:harvest_level":
        ...
        elif condition_type == "origins:nbt":
        ...
        elif condition_type == "origins:meat":
        ...
        if condition_type == "origins:is_damageable":
            json_data["type"] = "origins:damageable"
            log("INFO", trace, "Renamed is_damageable to damageable")
        if condition_type == "origins:is_equippable":
            json_data["type"] = "origins:equippable"
            log("INFO", trace, "Renamed is_equippable to equippable")
        if condition_type == "origins:fireproof":
            json_data["type"] = "origins:fire_resistant"
            log("INFO", trace, "Renamed fireproof to fire_resistant")


def fix_damage_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Damage")
    elif condition_type in docconditions.damage_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.damage_conditions)


def fix_biome_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Biome")
    elif condition_type in docconditions.biome_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.biome_conditions)
        if condition_type == "origins:category":
...
def fix_fluid_condition(trace: TRACE_TYPE, json_data: dict[str, Any]):
    condition_type = get_type(json_data)
    if condition_type in docconditions.meta_conditions:
        fix_meta_condition(trace.copy(), condition_type, json_data)
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.meta_conditions, "Fluid")
    elif condition_type in docconditions.fluid_conditions:
        iterate_through_fields(trace.copy(), condition_type, json_data, docconditions.fluid_conditions)


def fix_attribute(trace: TRACE_TYPE, json_data: dict[str, Any]):
...
def fix_operation(trace: TRACE_TYPE, json_data: dict[str, Any]):
...
def fix_attributed_operation(trace: TRACE_TYPE, json_data: dict[str, Any]):
    if "addition" == json_data["operation"]:
...
def fix_value(trace: TRACE_TYPE, json_data: dict[str, Any]):
    if "value" in json_data:
        json_data = rename_key(json_data, "value", "amount")
        log("INFO", trace, "Renamed value to amount")


def fix_attribute_modifier(trace: TRACE_TYPE, json_data: dict[str, Any]):
    fix_attributed_operation(trace, json_data)
    fix_value(trace, json_data)


def fix_attributed_attribute_modifier(trace: TRACE_TYPE, json_data: dict[str, Any]):
...
def fix_status_effect_instance(trace: TRACE_TYPE, json_data: dict[str, Any]):
    if "effect" in json_data:
...
def fix_food_component(trace: TRACE_TYPE, json_data: dict[str, Any]):
...
def fix_crafting_recipe(trace: TRACE_TYPE, json_data: dict[str, Any]):
    log("ERROR", trace, "Fixing crafting recipes is unimplemented.")


def fix_particle_effect(trace: TRACE_TYPE, json_data: dict[str, Any]):
    try:
        old_params: str = json_data['params']
    except KeyError:
        return
...
def select_type(trace: TRACE_TYPE, data_type: str, field_data: dict[str, Any] | Any, meta_type: Optional[str] = None):
    if data_type == "Entity Action Type" and isinstance(field_data, dict):
        fix_entity_action(trace.copy(), field_data)
    if data_type == "Bi-entity Action Type" and isinstance(field_data, dict):
        fix_bientity_action(trace.copy(), field_data)
    if data_type == "Block Action Type" and isinstance(field_data, dict):
        fix_block_action(trace.copy(), field_data)
    if data_type == "Item Action Type" and isinstance(field_data, dict):
        fix_item_action(trace.copy(), field_data)
    if data_type == "Entity Condition Type" and isinstance(field_data, dict):
        fix_entity_condition(trace.copy(), field_data)
    if data_type == "Bi-entity Condition Type" and isinstance(field_data, dict):
        fix_bientity_condition(trace.copy(), field_data)
    if data_type == "Block Condition Type" and isinstance(field_data, dict):
        fix_block_condition(trace.copy(), field_data)
    if data_type == "Item Condition Type" and isinstance(field_data, dict):
        fix_item_condition(trace.copy(), field_data)
    if data_type == "Damage Condition Type" and isinstance(field_data, dict):
        fix_damage_condition(trace.copy(), field_data)
    if data_type == "Biome Condition Type" and isinstance(field_data, dict):
        fix_biome_condition(trace.copy(), field_data)
    if data_type == "Fluid Condition Type" and isinstance(field_data, dict):
        fix_fluid_condition(trace.copy(), field_data)
    if data_type == "Attribute Modifier" and isinstance(field_data, dict):
        fix_attribute_modifier(trace.copy(), field_data)
    if data_type == "Attributed Attribute Modifier" and isinstance(field_data, dict):
        fix_attributed_attribute_modifier(trace.copy(), field_data)
    if data_type == "Status Effect Instance" and isinstance(field_data, dict):
        fix_status_effect_instance(trace.copy(), field_data)
    if data_type == "Food Component" and isinstance(field_data, dict):
        fix_food_component(trace.copy(), field_data)
    if data_type == "Crafting Recipe" and isinstance(field_data, dict):
        fix_crafting_recipe(trace.copy(), field_data)
    if data_type == "Particle Effect" and isinstance(field_data, dict):
        fix_particle_effect(trace.copy(), field_data)
    if data_type == "Action Type" and isinstance(field_data, dict):
        select_type(trace, meta_type + " Action Type", field_data)
    if data_type == "Condition Type" and isinstance(field_data, dict):
...
# then fixes it
def find_allowed_types(trace: TRACE_TYPE, allowed_types: list[dict[str, str | bool]], field_data: list[dict[str, Any]] | dict[str, Any], meta_type: Optional[str] = None):
    for data_type in allowed_types:
        if data_type["is_array"]:
            for i, json_object in enumerate(field_data):
                new_trace = trace.copy()
                new_trace["fields"] = new_trace["fields"] + "[" + str(i) + "]"
                select_type(new_trace, data_type["type"], json_object, meta_type)
        else:
            select_type(trace.copy(), data_type["type"], field_data, meta_type)
    return field_data


# Detects the type and iterates through that type's fields, fixing each
def iterate_through_fields(trace: TRACE_TYPE, field_type: str, json_data: dict[str, Any], shape_data: dict[str, list[dict[str, str | list]]], meta_type: Optional[str] = None):
    shape = shape_data[field_type]
...
def fix_power(trace: TRACE_TYPE, json_data: dict[str, Any]):
    ...
    iterate_through_fields(trace.copy(), power_type, json_data, docpowers.powers)

    if power_type == "origins:entity_group":
...
def update_powers(trace: TRACE_TYPE, folder_path: Path):
    ...
    for file in files:
        ...
        power_type = get_type(json_data)
        trace["fields"] = ""
        if power_type == "origins:multiple":
            shape = docpowers.powers[power_type]
...
def fix_item_stack(trace: TRACE_TYPE, stack: dict[str, str | int | dict]):
...
def fix_icon(trace: TRACE_TYPE, origin: dict[str, Any]):
    icon = origin["icon"]
...
def update_origins(trace: TRACE_TYPE, folder_path: Path):
...
def update_folders(trace: TRACE_TYPE, path: Path):
